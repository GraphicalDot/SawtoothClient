...
def check_hash(file_bytes, hash):
        ...
        if calculated_hash != hash:
            raise ApiBadRequest("File hash doesnt Match, Please send the right sha3_224 hash")
        return True
...
async def get_child_keys(request, data):
    async with aiohttp.ClientSession() as session:
            async with session.post(f"http://{request.app.config.REGISTRATION}/child_mnemonic_keys",
                json=data) as request_response:
